...
tok_status_t tokenizer_relex_text(tokenizer_t *tok, const char *text)
{
    ...
    if (num_new_tokens > 0)
    {
        ...
        int result = token_list_insert_range(tok->input_tokens, tok->input_pos, detached_tokens,
                                             detached_size);
...
tok_status_t tokenizer_process_one_token(tokenizer_t *tok)
{
    ...
    if (token_get_type(token) == TOKEN_WORD)
    {
        ...
        if (tok->input_pos + 1 < token_list_size(tok->input_tokens))
        {
            ...
            bool is_redir = (next_type == TOKEN_LESS || next_type == TOKEN_GREATER ||
                             next_type == TOKEN_DGREAT || next_type == TOKEN_DLESS ||
                             next_type == TOKEN_DLESSDASH || next_type == TOKEN_LESSAND ||
                             next_type == TOKEN_GREATAND || next_type == TOKEN_LESSGREAT ||
