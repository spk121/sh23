...
static int check_command_file_readable(const char *command_file)
{
    ...
    // Check if file is readable
    if ((_access(command_file, 4) != 0) || (_access(command_file, 6) != 0))
    {
        return SH_EXIT_COMMAND_FILE_READ_ERROR;
    }
#else
...
int main(int argc, char *argv[])
{
    // In ISO C, argc can be zero and argv NULL,
    // but if they aren't, argv[0] is the program name.
    // Even so, argv[0][0] can be '\0' indicating no name even if argc > 0.
