...
CTEST(test_parser_simple_command)
{
    ...
    if (ast != NULL)
    {
        ...
        if (ast_node_get_type(ast) == AST_PROGRAM)
        {
            ...
            if (ast_node_get_type(cmd_list) == AST_COMMAND_LIST)
            {
                ...
                if (ast_node_get_type(first) == AST_SIMPLE_COMMAND)
...
CTEST(test_parser_simple_command_with_args)
{
    ...
    if (ast != NULL && ast_node_get_type(ast) == AST_PROGRAM)
    {
        ast_node_t* cmd_list = ast->data.program.body;
        CTEST_ASSERT_EQ(ctest, ast_node_get_type(cmd_list), AST_COMMAND_LIST, "body is command list");
        if (ast_node_get_type(cmd_list) == AST_COMMAND_LIST)
...
CTEST(test_parser_pipeline)
{
    ...
    if (ast != NULL && ast_node_get_type(ast) == AST_PROGRAM)
    {
        ...
        if (ast_node_get_type(cmd_list) == AST_COMMAND_LIST)
        {
            ...
            if (ast_node_get_type(first) == AST_PIPELINE)
...
CTEST(test_parser_sequential_commands)
{
    ...
    if (ast != NULL && ast_node_get_type(ast) == AST_PROGRAM)
    {
        ...
        if (ast_node_get_type(cmd_list) == AST_COMMAND_LIST)
        {
            CTEST_ASSERT_EQ(ctest, ast_node_get_type(cmd_list), AST_COMMAND_LIST, "is command list");
