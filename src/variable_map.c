...
static void set_entry(variable_map_entry_t *dest, const string_t *key, const variable_map_mapped_t *mapped)
{
...
int32_t variable_map_insert_or_assign(variable_map_t *map, const string_t *key, variable_map_mapped_t *mapped)
{
    Expects_not_null(map);
    Expects_not_null(key);
    Expects_not_null(mapped);
    ...
    if (map->size >= map->capacity * 3 / 4)
    {
        /* When the size of the map changes, need to re-hash
