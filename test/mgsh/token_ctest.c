...
CTEST(token_recompute_flags_basic) {
    ...
    string_t *lit = string_create_cstr("hello");
    token_append_literal(tok, lit);
    string_destroy(&lit);

    token_recompute_flags(tok);
    ...
    string_t *param = string_create_cstr("USER");
...
CTEST(part_list_remove_basic) {
    ...
    string_t *lit1 = string_create_cstr("foo");
    string_t *lit2 = string_create_cstr("bar");
