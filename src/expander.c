...
static string_t *expand_parameter(expander_t *exp, const part_t *part)
{
    ...
    case PARAM_USE_DEFAULT: {
        ...
        else
        {
            ...
            if (word == NULL)
                return string_create();
            string_t *expanded_word = expander_expand_word_string(
                exp, word, part_was_single_quoted(part), part_was_double_quoted(part));
    ...
    case PARAM_ERROR_IF_UNSET: {
        ...
        else
        {
            ...
            if (word != NULL && string_length(word) > 0)
            {
                ...
                {
                    char err_msg[256];
                    snprintf(err_msg, sizeof(err_msg), "%s: parameter not set",
                             string_cstr(param_name));
    ...
    case PARAM_REMOVE_SMALL_SUFFIX: {
        ...
        if (pattern != NULL)
        {
            string_t *expanded_pattern = expander_expand_word_string(
                exp, pattern, part_was_single_quoted(part), part_was_double_quoted(part));
            string_t *result =
                remove_suffix_smallest(value, expanded_pattern ? expanded_pattern : pattern);
            if (expanded_pattern)
    ...
    case PARAM_REMOVE_LARGE_SUFFIX: {
        ...
        if (pattern != NULL)
        {
            string_t *expanded_pattern = expander_expand_word_string(
                exp, pattern, part_was_single_quoted(part), part_was_double_quoted(part));
            string_t *result =
                remove_suffix_largest(value, expanded_pattern ? expanded_pattern : pattern);
            if (expanded_pattern)
                string_destroy(&expanded_pattern);
            string_destroy(&value);
            return result ? result : string_create();
    ...
    case PARAM_REMOVE_SMALL_PREFIX: {
        ...
        if (pattern != NULL)
        {
            string_t *expanded_pattern = expander_expand_word_string(
                exp, pattern, part_was_single_quoted(part), part_was_double_quoted(part));
            string_t *result =
                remove_prefix_smallest(value, expanded_pattern ? expanded_pattern : pattern);
            if (expanded_pattern)
                string_destroy(&expanded_pattern);
            string_destroy(&value);
            return result ? result : string_create();
    ...
    case PARAM_REMOVE_LARGE_PREFIX: {
        ...
        if (pattern != NULL)
        {
            string_t *expanded_pattern = expander_expand_word_string(
                exp, pattern, part_was_single_quoted(part), part_was_double_quoted(part));
            string_t *result =
                remove_prefix_largest(value, expanded_pattern ? expanded_pattern : pattern);
            if (expanded_pattern)
                string_destroy(&expanded_pattern);
            string_destroy(&value);
            return result ? result : string_create();
...
static string_t *expand_command_substitution(expander_t *exp, const part_t *part)
{
    ...
    string_t *expanded_command = expander_expand_word_string(
        exp, command, part_was_single_quoted(part), part_was_double_quoted(part));

    log_debug("expand_command_substitution: invoking callback for command: %s",
              string_cstr(expanded_command));
    string_t *result = exp->cmd_subst_callback(expanded_command, exp->cmd_subst_executor_ctx,
...
static string_t *expand_arithmetic(expander_t *exp, const part_t *part)
{
    ...
    string_t *expanded_expr = expander_expand_word_string(
        exp, expr_text, part_was_single_quoted(part), part_was_double_quoted(part));
...
static void expand_word_part_tilde(const part_t *part, string_t *expanded, bool is_quoted,
                                   bool at_word_start)
{
    const string_t *text = part_get_text(part);
    if (text == NULL)
...
static bool expand_word_part_parameter(expander_t *exp, const part_t *part, string_t *expanded,
                                       bool is_quoted)
{
...
static bool expand_word_part_command_subst(expander_t *exp, const part_t *part, string_t *expanded,
                                           bool is_quoted)
{
    bool produced_unquoted_expansion = false;
...
static bool expand_word_part_arithmetic(expander_t *exp, const part_t *part, string_t *expanded,
                                        bool is_quoted)
{
    bool produced_unquoted_expansion = false;
...
string_list_t *expander_expand_word(expander_t *exp, token_t *word_token)
{
    ...
    if (parts != NULL)
    {
        ...
        if (part_count == 1)
        {
            ...
            if (part_get_type(only_part) == PART_PARAMETER && part_was_double_quoted(only_part))
            {
                const string_t *param_name = part_get_param_name(only_part);
                if (param_name && string_length(param_name) == 1 && string_at(param_name, 0) == '@')
                {
        ...
        for (int i = 0; i < part_count; i++)
        {
            ...
            case PART_PARAMETER:
                part_unquoted_expansion =
                    expand_word_part_parameter(exp, part, expanded, is_quoted);
                break;

            case PART_COMMAND_SUBST:
                part_unquoted_expansion =
                    expand_word_part_command_subst(exp, part, expanded, is_quoted);
                break;

            case PART_ARITHMETIC:
                part_unquoted_expansion =
                    expand_word_part_arithmetic(exp, part, expanded, is_quoted);
                break;
    ...
    {
        log_debug("expander_expand_word: pathname expansion callback is set, checking %d fields",
                  string_list_size(result));
        ...
        for (int i = 0; i < field_count; i++)
        {
            ...
            if (has_glob_chars)
            {
                log_debug("expander_expand_word: found glob chars in field, calling "
                          "pathname_expansion_callback");
                // Invoke callback to expand the pattern
                string_list_t *matches =
                    exp->pathname_expansion_callback(field, exp->pathname_expansion_user_data);
